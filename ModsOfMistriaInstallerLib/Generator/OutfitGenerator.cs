...
public class OutfitGenerator
{
    ...
    // Kept here so compatibility diagnostics use exactly the same slot contract
    // as the generator. A legacy outfit is not invalid merely because it uses
    // momi/outfit; only an unknown slot cannot be generated by AIM.
    internal static bool TryGetFrameSize(string uiSlot, out int frameWidth, out int frameHeight)
    {
        if (SlotConfigs.TryGetValue(uiSlot, out var cfg))
        {
