...
public partial class NexusDownloadsViewModel : ViewModelBase
{
    ...
    /// <summary>
    /// A client for the saved key, or null when there is none.
    ///
    /// Callers that merely want to read a mod page - the conflict researcher, for instance - use
    /// this rather than <see cref="EnsureApiKeyAsync"/>: reading a description is a nicety, and
    /// interrupting the user with a key prompt for it would be out of proportion.
    /// </summary>
    public NexusApiClient? CreateApiClient() =>
        _nexusSettings.HasApiKey() ? new NexusApiClient(_nexusSettings.GetApiKey()!) : null;
    ...
    {
        if (!await EnsureApiKeyAsync())
            return new PatchInstallResult(false, false, Localization["GUINexusApiKeyMissing"]);
    ...
    private async Task<string> DescribeAccountAsync()
    {
        ...
        try
        {
            var user = await client.ValidateKeyAsync();
