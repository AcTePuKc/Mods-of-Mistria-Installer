...
public partial class ModlistCheckbox : UserControl
{
    ...
    public ModlistCheckbox()
    {
        ...
        // A row can be recycled out from under an open card while the pointer is still on it - the
        // list reloads after an install, or the search filter changes what is on screen.
        DetachedFromVisualTree += (_, _) => HoverCard.Hide();
        PointerExited += (_, _) => HoverCard.Hide();
    }

    /// <summary>See <see cref="HoverCard"/> for why these badges do not use a ToolTip.</summary>
    ...
    /// card is bound to the property being filled in, so it appears when the notes arrive.
    /// </summary>
    private void ChangelogHovered(object? sender, PointerEventArgs e)
    {
        if (DataContext is ModModel model) model.Commands?.LoadChangelogPreview.Execute(model);
        HoverCard.Show(sender);
    }
